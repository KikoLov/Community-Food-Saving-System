  ...
  <div>
    ...
    <div class="card mb-4">
      <div class="card-header">
        <i class="fas fa-table me-2"></i>订单列表
      </div>
      <div class="card-body">
          ...
          <table class="table table-bordered table-hover" id="ordersTable">
            ...
            <tbody>
              <tr v-for="row in pagedOrders" :key="row.orderId">
                <td><small class="text-muted">{{ row.orderNo }}</small></td>
                <td>
                  <strong>{{ row.productName }}</strong>
                  <span class="badge bg-secondary ms-1">x{{ row.quantity }}</span>
                </td>
                <td>{{ row.userId }}</td>
                <td>{{ row.merchantId }}</td>
                <td>
                  <span class="fw-bold text-danger">¥{{ row.totalAmount }}</span>
                </td>
                <td>
                  <span class="badge bg-warning text-dark font-monospace">{{ row.verifyCode }}</span>
                </td>
                <td>
                ...
                <td>
                  <button class="btn btn-sm btn-outline-primary" @click="openDetail(row)">查看详情</button>
                </td>
              </tr>
        ...
        <div v-if="filteredOrders.length > 0" class="pager-wrap mt-3">
    ...
    <div v-if="detailOrder" class="detail-mask" @click.self="detailOrder = null">
      <div class="detail-card">
        ...
        <div class="detail-grid">
          ...
          <div><span class="k">买家ID</span><span class="v">{{ detailOrder.userId }}</span></div>
          <div><span class="k">商户ID</span><span class="v">{{ detailOrder.merchantId }}</span></div>
          ...
          <div><span class="k">核销码</span><span class="v font-monospace">{{ detailOrder.verifyCode }}</span></div>
...
const exportCsv = () => {
  ...
  const header = ['订单号', '商品信息', '买家ID', '商户ID', '金额', '核销码', '状态', '下单时间']
